...
const char * const BITCOINRECO_CONF_FILENAME = "bitcoinreco.conf";
const char * const BITCOINRECO_PID_FILENAME = "bitcoinrecod.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinrecod -foo=bar
...
#else
    const char* pszModule = "bitcoinreco";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitcoinreco
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitcoinreco
    // Mac: ~/Library/Application Support/Bitcoinreco
    // Unix: ~/.bitcoinreco
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoinreco";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitcoinreco";
#else
    // Unix
    return pathRet / ".bitcoinreco";
#endif
#endif
}
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINRECO_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Bitcoinreco Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoinreco Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bitcoinreco Core developers";
    }
