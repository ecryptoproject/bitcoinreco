...
#if defined(NDEBUG)
# error "Bitcoinreco cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Bitcoinreco Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BREC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight==1)
    {
        nSubsidy = 38000050 * COIN; //Create 38 million coins to distribute 17 million to existing 
                                    //Bitcoin owner, about 6 million to existing RECO owners 
                                    //5 Million go to an Investor who provided the money to list the coin on an exchange and who made this project possible and the rest of 10 Million to fund the development of the project in the future
    } else
    {
        nSubsidy >>= halvings;
    }
...
void ThreadScriptCheck() {
    RenameThread("bitcoinreco-scriptch");
