...
void generateoneblock(const JSONRPCRequest& request)
{
    LogPrintf("Generateone block\n");
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return;
    }

    LogPrintf("Generateone block 2\n");
    int num_generate = 1;
    ...
    LogPrintf("Generateone block 3\n");
    generateBlocks(coinbase_script, num_generate, max_tries, true);
    LogPrintf("Generateone block END\n");
}
...
static UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nReturns a new Bitcoinreco address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"address\"    (string) The new bitcoinreco address\n"
            "\nExamples:\n"
    ...
    if (!pwallet->GetKeyFromPool(newKey2)) {
        throw JSONRPCError(RPC_WALLET_KEYPOOL_RAN_OUT, "Error: Keypool ran out, please call keypoolrefill first");
    }
...
/*
//read in UTXO Set of Bitcoin Blockchain to build the initial 5040 blocks
static UniValue readinblockchain(const JSONRPCRequest& request)
{
    ...
    CCoinControl coin_control;
    int64_t i_dec = 0;
    fs::ifstream optionFile2("/home/cadkas/outputsStatus.txt");
    if (optionFile2.good()){
    getline(optionFile2, line);
    line.erase(line.find_last_not_of(" \n\r\t")+1);
        LogPrintf("Start Line: %s\n", line);     
    i_dec = std::stoll (line);
        LogPrintf("Start Line: %d\n", i_dec);     
    optionFile2.close();
    }

    fs::ifstream optionFile("/home/cadkas/outputstotal.txt");
    if (!optionFile.good())
        return NullUniValue;
    CAmount nValue,summe;
    ...
    int trans=0;
    int transtotal=0;
    int64_t sumtranstotal=0;
    summe=0;
    // get length of file:
    optionFile.seekg (0, optionFile.end);
    int64_t length = optionFile.tellg();
    if (i_dec<length) {
    optionFile.seekg(i_dec);
    
    while (!optionFile.eof())
    {
        getline(optionFile, line);if (optionFile.eof()) break;
        getline(optionFile, line);if (optionFile.eof()) break;
        getline(optionFile, line);if (optionFile.eof()) break;
        getline(optionFile, line);if (optionFile.eof()) break;
        line.erase(line.find_last_not_of(" \n\r\t")+1);
//        LogPrintf("XY: %s transactions read in\n", line);
        while (line.size()<9) {line.insert(0,"0");}
        line.insert(line.end()-8,'.');
//        LogPrintf("YZ: %s transactions read in\n", line);
        nValue=AmountFromValue(ValueFromString(line));
        summe+=nValue;
        ...
        std::vector<unsigned char> scriptData(ParseHexV(line, "argument"));
        scriptPubKey = CScript(scriptData.begin(), scriptData.end());


        // get the scriptPubKey corresponding to this input:
        const CScript& prevScript = scriptPubKey;
        if (Solver(prevScript, whichType, vSolutions)){//Standard ScriptPubKey?
    ...
    if (trans>=1000)
    {
      ...
      CValidationState state;
      if (!pwallet->CommitTransaction(tx, std::move(mapValue), {} , std::move(fromAccount), reservekey, g_connman.get(), state)) {
        strError = strprintf("Error: The transaction was rejected! Reason given: %s", FormatStateMessage(state));
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
      } 
      vecSend.clear(); 

       //generateoneblock(request);
//       generateoneblock(request);
       trans=0;
       //break;
    }
    if (transtotal>=10000) {
        transtotal=0;
        LogPrintf("Readinblockchain: %ll transactions read in Pos: %ll of %ll in file\n", sumtranstotal,optionFile.tellg(),length);
        generateoneblock(request);
        LogPrintf("End generateone block\n");

    boost::filesystem::ofstream myfile;
    myfile.open ("/home/cadkas/outputsStatus.txt",std::ios::out | std::ios::trunc);
    myfile << (optionFile.tellg()) << "\n";
    myfile.close();
        break;
    }
    ...
    if (!pwallet->CommitTransaction(tx, std::move(mapValue), {}, std::move(fromAccount), reservekey, g_connman.get(), state)) {
        ...
        LogPrintf("Readinblockchain: %i transactions read in\n", sumtranstotal);
        generateoneblock(request);
        LogPrintf("End generateone block\n");
...
static UniValue getlabeladdress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "getaccountaddress") {
        if (request.fHelp) {
            throw std::runtime_error("getaccountaddress (Deprecated, will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccountaddress is deprecated and will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts.");
    }
...
static UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new Bitcoinreco address, for receiving change.\n"
    ...
    if (!reservekey.GetReservedKey(vchPubKey, true))
        throw JSONRPCError(RPC_WALLET_KEYPOOL_RAN_OUT, "Error: Keypool ran out, please call keypoolrefill first");
    reservekey.KeepKey();
    if (!reservekey.GetReservedKey(vchPubKey2, true))
        throw JSONRPCError(RPC_WALLET_KEYPOOL_RAN_OUT, "Error: Keypool ran out, please call keypoolrefill first");
...
static UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "setaccount") {
        if (request.fHelp) {
            throw std::runtime_error("setaccount (Deprecated, will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "setaccount is deprecated and will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitcoinreco address to be associated with a label.\n"
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bitcoinreco address");
    }
...
static UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccount (Deprecated, will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccount is deprecated and will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitcoinreco address for account lookup.\n"
            "\nResult:\n"
...
static UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaddressbyaccount (Deprecated, will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaddressesbyaccount is deprecated and will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a bitcoinreco address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 9)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The bitcoinreco address to send to.\n"
            ...
            "                             The recipient will receive less bitcoinrecos than you enter in the amount field.\n"
...
static UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The bitcoinreco address\n"
...
static UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitcoinreco address to use for the private key.\n"
...
static UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitcoinreco address for transactions.\n"
...
static UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "getreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("getreceivedbyaccount (Deprecated, will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getreceivedbyaccount is deprecated and will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts.");
    }
...
static UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() > 3 && IsDeprecatedRPCEnabled("accounts")) || (request.params.size() != 0 && !IsDeprecatedRPCEnabled("accounts")))
        throw std::runtime_error(
            ...
            "                     To use this deprecated argument, start bitcoinrecod with -deprecatedrpc=accounts. The account string may be given as a\n"
            ...
            "2. minconf           (numeric, optional, default=1) DEPRECATED. Only valid when an account is specified. This argument will be removed in V0.18. To use this deprecated argument, start bitcoinrecod with -deprecatedrpc=accounts. Only include transactions confirmed at least this many times.\n"
            "3. include_watchonly (bool, optional, default=false) DEPRECATED. Only valid when an account is specified. This argument will be removed in V0.18. To use this deprecated argument, start bitcoinrecod with -deprecatedrpc=accounts. Also include balance in watch-only addresses (see 'importaddress')\n"
            "\nResult:\n"
...
static UniValue movecmd(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("move (Deprecated, will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "move is deprecated and will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts.");
    }
...
static UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a bitcoinreco address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The bitcoinreco address to send funds to.\n"
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "sendmany \"\" {\"address\":amount,...} ( \"referenceline\" minconf \"comment\" [\"address\",...] replaceable conf_target \"estimate_mode\")\n"
            ...
            "bitcoinrecod with -deprecatedrpc=accounts\n"
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"dummy\"               (string, required) Must be set to \"\" for backwards compatibility.\n"
            "2. \"amounts\"             (string, required) A json object with addresses and amounts\n"
            "    {\n"
            "      \"address\":amount   (numeric or string) The bitcoinreco address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            "3. \"referenceline\"       (string, optional) A reference line used to store what the transaction is for. \n"
            "                             This is part of the transaction.\n"
            ...
            "                           Those recipients will receive less bitcoinrecos than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Bitcoinreco address: ") + name_);
        }
...
static UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"label\" \"address_type\" )\n"
            ...
            "Each key is a Bitcoinreco address or hex-encoded public key.\n"
            ...
            "2. \"keys\"                         (string, required) A json array of bitcoinreco addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"                  (string) bitcoinreco address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
...
static UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart bitcoinrecod with -deprecatedrpc=addwitnessaddress.\n"
...
static UniValue listreceivedbylabel(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "listreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("listreceivedbyaccount (Deprecated, will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listreceivedbyaccount is deprecated and will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts.");
    }
...
static void ListTransactions(CWallet* const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        for (const COutputEntry& s : listSent)
        {
            ...
            entry.pushKV("referenceline", pwallet->DecryptRefLine2PubKeys(s.referenceline,s.senderPK,s.receiverPK));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        for (const COutputEntry& r : listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.pushKV("referenceline", pwallet->DecryptRefLine2PubKeys(r.referenceline,r.senderPK,r.receiverPK));
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "listtransactions (dummy count skip include_watchonly)\n"
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"address\",    (string) The bitcoinreco address of the transaction.\n"
        ...
        help_text = "listtransactions ( \"account\" count skip include_watchonly)\n"
            ...
            "    \"address\":\"address\",    (string) The bitcoinreco address of the transaction. Not present for \n"
...
static UniValue listaccounts(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("listaccounts (Deprecated, will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listaccounts is deprecated and will be removed in V0.18. To use this command, start bitcoinrecod with -deprecatedrpc=accounts.");
    }
...
static UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"account\":\"accountname\",       (string) DEPRECATED. This field will be removed in V0.18. To see this deprecated field, start bitcoinrecod with -deprecatedrpc=accounts. The account name associated with the transaction. Will be \"\" for the default account.\n"
            "    \"address\":\"address\",    (string) The bitcoinreco address of the transaction. Not present for move transactions (category = move).\n"
...
static UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"account\" : \"accountname\",      (string) DEPRECATED. This field will be removed in a V0.18. To see this deprecated field, start bitcoinrecod with -deprecatedrpc=accounts. The account name involved in the transaction, can be \"\" for the default account.\n"
            "      \"address\" : \"address\",          (string) The bitcoinreco address involved in the transaction\n"
...
static UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending bitcoinrecos\n"
            "\nArguments:\n"
...
static UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending bitcoinreco\n"
    ...
    StartShutdown();
    return "wallet encrypted; Bitcoinreco server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
static UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending bitcoinrecos.\n"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of bitcoinreco addresses to filter\n"
            "    [\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the bitcoinreco address\n"
            "    \"label\" : \"label\",        (string) The associated label, or \"\" for the default label\n"
            "    \"account\" : \"account\",    (string) DEPRECATED. This field will be removed in V0.18. To see this deprecated field, start bitcoinrecod with -deprecatedrpc=accounts. The associated account, or \"\" for the default account\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Bitcoinreco address: ") + input.get_str());
            }
...
static UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The bitcoinreco address to receive the change\n"
    ...
    if (!request.params[1].isNull()) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid bitcoinreco address");
            }
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nReturn information about the given bitcoinreco address. Some information requires the address\n"
            "to be in the wallet.\n"
            "\nArguments:\n"
            "1. \"address\"                    (string, required) The bitcoinreco address to get the information of.\n"
            "\nResult:\n"
            "{\n"
            "  \"address\" : \"address\",        (string) The bitcoinreco address validated\n"
            ...
            "  \"account\" : \"account\"         (string) DEPRECATED. This field will be removed in V0.18. To see this deprecated field, start bitcoinrecod with -deprecatedrpc=accounts. The account associated with the address, \"\" is the default account\n"
...
extern UniValue dumpprivkey2(const JSONRPCRequest& request); // in rpcdump.cpp
extern UniValue importprivkey(const JSONRPCRequest& request);
extern UniValue importprivkey2(const JSONRPCRequest& request);
...
//    { "rawtransactions",    "readinblockchain",                 &readinblockchain,              {} },
    ...
    { "wallet",             "dumpprivkey2",                     &dumpprivkey2,                  {"address"}  },
    ...
    { "wallet",             "importprivkey2",                   &importprivkey2,                {"privkey1","privkey2","label","rescan"} },
