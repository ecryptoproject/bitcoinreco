...
bool CWallet::GetLabelDestination(CTxDestination &dest, const std::string& label, bool bForceNew)
{
    ...
    if (!bForceNew) {
        if (!account.vchPubKey.IsValid() || !account.vchPubKey2.IsValid())
            bForceNew = true;
        else {
            // Check if the current key has been used (TODO: check other addresses with the same key)
            CScript scriptPubKey = GetScriptForDestination(GetDestinationFor2Keys(account.vchPubKey, account.vchPubKey2, m_default_address_type));
    ...
    if (bForceNew) {
        if (!GetKeyFromPool(account.vchPubKey, false))
            return false;
        if (!GetKeyFromPool(account.vchPubKey2, false))
            return false;

        LearnRelatedScripts(account.vchPubKey, m_default_address_type);
        LearnRelatedScripts(account.vchPubKey2, m_default_address_type);
        dest = GetDestinationFor2Keys(account.vchPubKey,account.vchPubKey2, m_default_address_type);
...
std::string CWalletTx::DecryptRefLine2PubKeys(std::string referenceline,CPubKey pubkey1,CPubKey pubkey2) const
{
    return pwallet->DecryptRefLine2PubKeys(referenceline,pubkey1,pubkey2);
}
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/bitcoinreco/bitcoinreco/pull/10340, but that change has been
    // postponed until after https://github.com/bitcoinreco/bitcoinreco/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-bitcoinreco-address
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    if (recipient.refline!="")
		    GetKeyFromPool(senderpubkey,false);//do not need to get a new key for empty reference line
 ...
 * https://bitcoinrecotalk.org/?topic=54527, or
 * https://github.com/bitcoinreco/bitcoinreco/pull/1393.
 */
