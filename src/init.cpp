 ...
 * between bitcoinrecod, and bitcoinreco-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("bitcoinreco-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)"), BITCOINRECO_CONF_FILENAME));
    if (mode == HelpMessageMode::BITCOINRECOD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)"), BITCOINRECO_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitcoinreco/bitcoinreco>";
    const std::string URL_WEBSITE = "<https://bitcoinrecocore.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("bitcoinreco-loadblk");
...
/** Sanity checks
 *  Ensure that Bitcoinreco is running in a usable environment with all
...
{
    // Make sure only a single Bitcoinreco process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BITCOINRECO_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bitcoinreco is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitcoinreco is started while in a temporary directory.\n",
