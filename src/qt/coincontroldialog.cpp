    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/bitcoinreco/bitcoinreco/issues/5716
...
{
    GUIUtil::setClipboard(BitcoinrecoUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinrecoUnits::BREC;
    ...
    l2->setText(BitcoinrecoUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(BitcoinrecoUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(BitcoinrecoUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(BitcoinrecoUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            {
                sAddress = QString::fromStdString(EncodeDestination(outputAddress,out.txout.receiverPubKey));

                // if listMode or change => show bitcoinreco address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, BitcoinrecoUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, BitcoinrecoUnits::format(nDisplayUnit, nSum));
