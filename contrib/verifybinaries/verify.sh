...
###   bitcoinrecocore.org and bitcoinreco.org and compares them.
...
WORKINGDIR="/tmp/bitcoinreco_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinrecocore.org"
HOST2="https://bitcoinreco.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinreco-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinreco-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinreco.org failed to provide signature file, but bitcoinrecocore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinreco.org and bitcoinrecocore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoinreco Core binary release signing key installed?"
   fi
