...
The RPM spec file provided here is for Bitcoinreco-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoinreco.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoinreco git repository](https://github.com/bitcoinreco/bitcoinreco) but are not part of
...
Currently only `Source10` is used, for the example `bitcoinreco.conf` file.
...
in the bitcoinreco git repository. At present only an SVG version of the Bitcoinreco
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoinreco
...
needed when 0.12.1 is released, a proper fix is already in the Bitcoinreco git
master branch.
...
idea to build Bitcoinreco Core against a static version of that library compiled
...
The `bitcoinreco.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinreco-server` package. At this point, most RPM based
...
The `bitcoinreco-server` package should have SELinux support. How to properly do
...
Bitcoinreco-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinreco/). He has been
packaging Bitcoinreco for Fedora at least since 2012.
...
4. I renamed the `bitcoinreco` package that contains the Qt GUI to `bitcoinreco-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinreco-xt` or `bitcoinreco-classic`. I wanted to make sure the general population
